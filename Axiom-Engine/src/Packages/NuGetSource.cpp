...
namespace Axiom {
	...
	std::string NuGetSource::RunTool(const std::vector<std::string>& args) const {
		...
		if (!result.Succeeded()) {
			AIM_CORE_ERROR_TAG("NuGetSource", "Failed to run tool (exit code {})", result.ExitCode);
		}
		else if (result.Output.empty()) {
			AIM_CORE_WARN_TAG("NuGetSource", "Tool returned empty output");
		}
	...
	std::vector<PackageInfo> NuGetSource::Search(const std::string& query, int take) {
		...
		if (!Json::TryParse(output, root, &parseError) || !root.IsArray()) {
			if (!output.empty()) {
				AIM_CORE_WARN_TAG("NuGetSource", "Failed to parse search results: {}", parseError);
			}
			return results;
		}
