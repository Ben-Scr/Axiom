...
namespace Axiom {
		...
		TextureFormat colorFormat, TextureFilter /*filter*/)
	{
		...
		if (!WebGPUBackend::IsInitialized()) {
			AIM_CORE_ERROR_TAG("Framebuffer",
		...
		if (!color) {
			AIM_CORE_ERROR_TAG("Framebuffer", "wgpu::Device::CreateTexture (color) failed");
			return false;
		}
		...
		if (!depth) {
			AIM_CORE_ERROR_TAG("Framebuffer", "wgpu::Device::CreateTexture (depth) failed");
			return false;
		}
