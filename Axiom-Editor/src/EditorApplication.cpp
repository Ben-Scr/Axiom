...
Axiom::Application* Axiom::CreateApplication() {
	...
	if (!s_ProjectPath.empty() && AxiomProject::Validate(s_ProjectPath)) {
		auto project = std::make_unique<AxiomProject>(AxiomProject::Load(s_ProjectPath));
		AIM_CORE_INFO_TAG("Editor", "Opening project: {} ({})", project->Name, project->RootDirectory);
		ProjectManager::SetCurrentProject(std::move(project));
	}

	if (!ProjectManager::HasProject()) {
		AIM_CORE_WARN_TAG("Editor", "No project specified. Use: Axiom-Editor.exe --project=\"path/to/project\"");
		AIM_CORE_WARN_TAG("Editor", "Starting with default fallback paths.");
	}
